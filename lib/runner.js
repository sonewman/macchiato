...
function Runner(options) {
  ...
  self.run = function (currentBlock, done) {
    ...
    currentBlock.forEach(perChild, testComplete)
  }
}
...
Runner.prototype.walk = function (child) {
  ...
  inheritParentsAspects(child, parent)
...
Runner.prototype.gatherFromBase = function (newBlock) {
  ...
  addNewBlock(current, newBlock)

  function then() {
    /** jshint validthis: true */
    self.baseGatherComplete = true
    ...
    self.baseWalk(current, finished)
  }

  function canStart() {
    return !current.started
      && ++self._startDelayCount === self.startDelay
  }
